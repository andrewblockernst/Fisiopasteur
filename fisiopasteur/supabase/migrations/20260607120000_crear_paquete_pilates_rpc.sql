-- Crea un paquete de Pilates atómicamente: múltiples pacientes × múltiples slots
-- en una sola transacción server-side. Inspirado en `crear_paquete_sesiones_rpc`,
-- pero adaptado a la semántica de Pilates (varios pacientes comparten el mismo
-- slot con el mismo especialista; capacidad máxima 4 por slot).
...
begin
  ...
  -- Validar capacidad: existentes en el slot (misma especialidad Pilates) + nuevos <= 4.
  with payload as (
    select
      (t ->> 'fecha')::date as fecha,
      (t ->> 'hora')::time as hora
  ...
  -- (el trigger validar_turno_unico_por_paciente también lo enforza, pero un
  -- mensaje agregado es más útil para el cliente).
  with payload as (
    select
      (t ->> 'fecha')::date as fecha,
      (t ->> 'hora')::time as hora
  ...
  pacientes as (
    select unnest(p_id_pacientes) as id_paciente
  ),
  expandido as (
    select pa.id_paciente, p.fecha, p.hora
    from pacientes pa cross join payload p
  ),
  ya_tienen as (
    select
  ...
  loop
    ...
    foreach v_paciente_id in array p_id_pacientes loop
