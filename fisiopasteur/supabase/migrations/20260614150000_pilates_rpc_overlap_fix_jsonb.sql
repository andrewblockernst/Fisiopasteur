-- Fix de la migración previa 20260614140000: creé un OVERLOAD nuevo con
-- `p_id_pacientes integer[]` en lugar de modificar la variante canónica
-- `jsonb`. Resultado: dos funciones con la misma cantidad de parámetros y
-- PostgREST devuelve "Could not choose the best candidate function".
--
-- Solución:
--   1) Drop de la overload `integer[]` errónea (la que dejé tras 14000000).
--   2) Reemplazo de la versión `jsonb` (la que el cliente realmente usa)
--      aplicando la lógica de overlap H-30 / H / H+30 que era el objetivo.
...
begin
  ...
  -- NUEVO: solape de 30 min con cualquier clase de Pilates ya existente.
  -- Una clase dura 1 hora; H, H-30 y H+30 colisionan. Ignora especialista,
  -- paciente y cantidad de participantes — basta con que exista un turno
  -- activo de Pilates en la ventana para bloquear el slot.
  with payload as (
    select
      (t ->> 'fecha')::date as p_fecha,
      (t ->> 'hora')::time as p_hora
  ...
  -- Capacidad por slot (defensa en profundidad; con el chequeo de solape ya
  -- nunca debería disparar para conflictos contra clases existentes).
  with payload as (
    select
      (t ->> 'fecha')::date as p_fecha,
      (t ->> 'hora')::time as p_hora
