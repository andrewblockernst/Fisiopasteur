-- Fix: las columnas `fecha`/`hora` del RETURNS TABLE de `crear_paquete_pilates_rpc`
-- se declaran como variables locales en PL/pgSQL y chocan con los nombres
-- `fecha`/`hora` usados en los CTEs internos ("column reference is ambiguous"
-- — SQLSTATE 42702).
--
-- Solución: prefijar las variables de salida con `out_` y asignarlas al final
-- de cada iteración. Mantiene la misma firma pública y semántica.
