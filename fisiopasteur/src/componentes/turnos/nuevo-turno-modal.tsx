...
export default function NuevoTurnoDialog({ open, onClose, onCreated }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [p, e, b] = await Promise.all([
        obtenerPacientes(),         // ya la tenés
        obtenerEspecialistas(),     // ya la tenés
        obtenerBoxes(),             // ya la tenés
      ]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setDisponible(res.success ? res.disponible : null);
    })();
  }, [fecha, hora, especialistaId, boxId]);

  const onSubmit = () => {
    if (!pacienteId || !especialistaId || !fecha || !hora) {
      alert("Completá paciente, especialista, fecha y hora.");
      return;
    }
    if (disponible === false) {
