import { utils as utils$d, ProviderClass } from '@builderbot/bot';
import require$$0$5, { createWriteStream, existsSync, createReadStream, readFileSync as readFileSync$1 } from 'fs';
...
import { readdir, unlink, writeFile as writeFile$2 } from 'fs/promises';
import require$$1$2 from 'events';
import require$$0$8, { tmpdir } from 'os';
import require$$1$3 from 'worker_threads';
import require$$0$7 from 'module';
...
class BaileysProvider extends ProviderClass {
    constructor(args) {
                ...
                const sock = makeWASocket({
                    ...
                    patchMessageBeforeSending: (message) => {
                        if (message.deviceSentMessage?.message?.listMessage?.listType ===
                            proto$2.Message.ListMessage.ListType.PRODUCT_LIST) {
                            message = JSON.parse(JSON.stringify(message));
                            message.deviceSentMessage.message.listMessage.listType =
                                proto$2.Message.ListMessage.ListType.SINGLE_SELECT;
                        }
                        if (message.listMessage?.listType == proto$2.Message.ListMessage.ListType.PRODUCT_LIST) {
                            message = JSON.parse(JSON.stringify(message));
                            message.listMessage.listType = proto$2.Message.ListMessage.ListType.SINGLE_SELECT;
                        }
                        return message;
                    },
                    ...this.globalVendorArgs,
                    ...
                    for (const messageCtx of messages) {
                        ...
                        if (messageCtx.message?.locationMessage) {
                            ...
                            if (typeof degreesLatitude === 'number' && typeof degreesLongitude === 'number') {
                                payload = {
                                    ...payload,
                                    body: utils$d.generateRefProvider('_event_location_'),
                                };
                            }
                        }
                        if (messageCtx.message?.videoMessage) {
                            payload = { ...payload, body: utils$d.generateRefProvider('_event_media_') };
                        }
                        ...
                        if (messageCtx.message?.documentMessage || messageCtx.message?.documentWithCaptionMessage) {
                            payload = { ...payload, body: utils$d.generateRefProvider('_event_document_') };
                        }
                        if (messageCtx.message?.audioMessage) {
                            payload = { ...payload, body: utils$d.generateRefProvider('_event_voice_note_') };
                        }
                        if (messageCtx.message?.orderMessage) {
                            payload = { ...payload, body: utils$d.generateRefProvider('_event_order_') };
                        }
                ...
                func: async ([call]) => {
                    if (call.status === 'offer') {
                        const payload = {
                            ...
                            body: utils$d.generateRefProvider('_event_call_'),
                            call,
                        };
                        this.emit('message', payload);
        ...
        this.saveFile = async (ctx, options) => {
            ...
            await writeFile$2(pathFile, buffer);
